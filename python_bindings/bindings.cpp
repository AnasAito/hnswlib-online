...
public:
    ...
    std::vector<hnswlib::labeltype> getNeighborsList(int nei_id)
    {
        ...
        for (size_t j = 1; j <= size; j++)
        {
            ...
            ids.push_back(appr_alg->getExternalLabel(candidate_id));
        }
        return ids;
    }
        ...
        const std::function<bool(hnswlib::labeltype)> &filter = nullptr)
    {
        ...
        return py::make_tuple(
            ...
            py::array_t<dist_t>(
                ...
                free_when_done_d));
        // py::array_t<hnswlib::labeltype>(
        //     {appr_alg->traversed_nodes_.size()}, // shape
        //     {sizeof(hnswlib::labeltype)},        // C-style contiguous strides for each index
        //     appr_alg->traversed_nodes_.data(),   // the data pointer
        //     py::capsule(appr_alg->traversed_nodes_.data(), [](void *) {})),
        // py::array_t<hnswlib::labeltype>(
        //     {appr_alg->current_result_queue.size()},                             // shape
        //     {sizeof(hnswlib::labeltype)},                                        // C-style contiguous strides for each index
        //     appr_alg->current_result_queue.data(),                               // the data pointer
        //     py::capsule(appr_alg->current_result_queue.data(), [](void *) {}))); // capsule with valid pointer
    }
